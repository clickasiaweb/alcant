...
const Logo = ({ size = "default", className = "" }) => {
  ...
  return (
    <div
      className={`font-bold text-primary-600 ${sizeClasses[size]} ${className}`}
    >
      ΛʟcΛɴᴛ
    </div>
  );
...
const AlcantaraHeader = () => {
    ...
    try {
      ...
      if (isMounted.current) {
        setCategoryProducts((prev) => ({
          ...prev,
          [categorySlug]: productsData.products || [],
        }));
      }
    } catch (error) {
  ...
  const fetchSubSubcategories = useCallback(
    async (categorySlug, subcategorySlug) => {
      try {
        if (!isMounted.current) return;
        setSubSubcategoriesLoading((prev) => ({
          ...prev,
          [subcategorySlug]: true,
        }));
        const subSubcategoriesData = await categoryService.getSubSubcategories(
          categorySlug,
          subcategorySlug,
        );
        if (isMounted.current) {
          setSubSubcategories((prev) => ({
            ...prev,
            [subcategorySlug]: subSubcategoriesData,
          }));
        }
      } catch (error) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const categoriesData =
          await categoryService.getCategoriesWithHierarchy();
  ...
  const handleDropdownEnter = useCallback(
    ...
    [fetchCategoryProducts, categoriesMap],
  ); // Use memoized categories map
  ...
  const handleKeyDown = (e, categoryName) => {
    switch (e.key) {
      ...
      case "ArrowUp":
        ...
        const prevIndex =
          (categories.findIndex((cat) => cat.name === categoryName) -
            1 +
            categories.length) %
          categories.length;
  ...
  const getCategoryIcon = (categoryName) => {
    ...
    if (name.includes("wallet") || name.includes("card"))
      return <Wallet className="w-5 h-5" />;
    if (name.includes("car") || name.includes("travel"))
      return <Car className="w-5 h-5" />;
    if (name.includes("bag") || name.includes("purse"))
  ...
  const getCategoryPromoImage = (categoryName) => {
    const name = categoryName.toLowerCase();
    if (name.includes("phone") || name.includes("case"))
      return "https://picsum.photos/seed/phone-cases-promo/400/500.jpg";
    if (name.includes("accessor"))
      return "https://picsum.photos/seed/accessories-promo/400/500.jpg";
    if (name.includes("wallet") || name.includes("card"))
      return "https://picsum.photos/seed/wallets-promo/400/500.jpg";
    if (name.includes("car") || name.includes("travel"))
      return "https://picsum.photos/seed/car-travel-promo/400/500.jpg";
    if (name.includes("bag") || name.includes("purse"))
      return "https://picsum.photos/seed/bags-promo/400/500.jpg";
            ...
            <nav className="hidden lg:flex items-center space-x-1 mega-menu-container">
              {categories.map((category) => (
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-3 h-3 sm:w-4 sm:h-4 transition-all duration-200 ml-1 ${
                        ...
                        >
                          <div className="px-6">
                            ...
                            {(() => {
                              ...
                              return subcategories[currentCategory.slug] &&
                                subcategories[currentCategory.slug].length >
                                  0 ? (
                                <div className="space-y-4" role="menu">
                                  {subcategories[currentCategory.slug].map(
                                    (subcategory, index) => (
                                      <div
                                      ...
                                      >
                                        ...
                                        >
                                          <div className="flex items-center space-x-2">
                                            <ChevronRight
                                              className={`w-3 h-3 transition-all duration-200 group-hover:translate-x-1 flex-shrink-0 ${
                                                hoveredSubcategory?.slug ===
                                                subcategory.slug
                                                  ? "text-primary-600"
                        ...
                        >
                          <div className="px-6">
                            {(() => {
                              ...
                              if (
                                hoveredSubcategory &&
                                subSubcategories[hoveredSubcategory.slug] &&
                                subSubcategories[hoveredSubcategory.slug]
                                  .length > 0
                              ) {
                                return (
                    ...
                    {getCurrentMobileView().map((item) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            if (hasSubCategories) {
                              navigateToMobileCategory(
                                item,
                                mobileNavStack.length,
                              );
                            } else {
                              router.push(item.href || "#");
                              setIsMobileMenuOpen(false);
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="w-10 h-10 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                              <img
                                src={
                                  item.image ||
                                  `https://via.placeholder.com/40x40/1a365d/ffffff?text=${encodeURIComponent((item.name || item.title).charAt(0))}`
                                }
                ...
                {mobileNavStack.length === 0 && (
                  <div className="mt-6 pt-6 border-t border-gray-200 space-y-3">
                    ...
                    <button
                      ...
                      onClick={() =>
                        alert(
                          "Login functionality is currently disabled for testing. Please use the cart and checkout features.",
                        )
                      }
                    >

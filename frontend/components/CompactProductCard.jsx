...
const CompactProductCard = ({ product, index = 0 }) => {
  ...
  return (
    <Link href={`/product-details/${product.slug}`} className="block">
      <div
        className="group cursor-pointer bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 w-full max-w-[320px] border border-gray-100"
      ...
      <div className="relative bg-gray-50 h-64 overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute top-3 left-3">
          {product.isBestseller && (
            <span className="bg-blue-600 text-white px-2.5 py-1 text-[11px] font-semibold rounded-full inline-block">
              Bestseller
            </span>
          )}
          {product.discount && !product.isBestseller && (
            <span className="bg-red-500 text-white px-2.5 py-1 text-[11px] font-semibold rounded-full inline-block">
              {product.discount}% Off
            </span>
          )}
          {product.isLimited && !product.isBestseller && !product.discount && (
            <span className="bg-purple-600 text-white px-2.5 py-1 text-[11px] font-semibold rounded-full inline-block">
              Limited Edition
            </span>
          )}
        </div>
      ...
      <div className="px-4 pt-3 pb-4">
        ...
        <Link href={`/product-details/${product.slug}`}>
          <h3 className="text-[14px] font-semibold text-gray-900 hover:text-blue-600 transition-colors leading-snug line-clamp-2 mb-2">
        ...
        <div className="flex items-center space-x-1 mb-3">
          ...
          <span className="text-[11px] text-gray-500">Magsafe Compatible</span>
        </div>
        ...
        <div className="flex items-center space-x-1.5 mb-4">
          ...
          <span className="text-[11px] text-gray-500 ml-0.5">+3</span>
        </div>

...
exports.createOrder = async (req, res) => {
  try {
    ...
    const orderData = {
      ...
      notes:
        notes ||
        `Order created with products: ${orderProducts.length} items. Subtotal: ${subtotal}, Tax: ${tax}, Shipping: ${shipping}, Discount: ${discountAmount}`,
    ...
    try {
      ...
      if (statusErr)
        console.warn("Failed to insert status history:", statusErr.message);
    } catch (e) {
    ...
    const responseOrder = {
      ...
      payment_status:
        order.payment_status || (paymentDetails?.paidAt ? "Paid" : "Pending"),
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    try {
      ...
      if (existingError) {
        console.warn(
          "Could not read existing status_history, continuing with fresh history entry:",
      ...
      console.warn(
        "Status history read threw error, continuing:",
        readError.message,
      );
      statusHistory = [];
    }
    ...
    if (!error && !order) {
      return res.status(403).json({
        success: false,
        message:
          "Status update blocked (likely RLS). Configure SUPABASE_SERVICE_ROLE_KEY on backend.",
      });
    }

...
exports.createProduct = async (req, res) => {
  try {
    ...
    // If images array is provided, set main image to first valid non-base64 image
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    if (updateData.images && Array.isArray(updateData.images) && updateData.images.length > 0) {
      ...
      if (firstImage && typeof firstImage === 'string') {
        if (isDataUrl(firstImage)) {
          // Do not overwrite existing image column with null during base64 updates.
          // Keep persisted image field unchanged.
          delete updateData.image;
          console.log('Primary image is base64; keeping existing image column unchanged');
        } else {
    ...
    if (error && error.code === 'NO_ROWS_UPDATED') {
      return res.status(500).json({
        error: "Product was not updated. It may be blocked by Supabase RLS or missing service role key.",

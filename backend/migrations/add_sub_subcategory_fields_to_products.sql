...
ADD COLUMN IF NOT EXISTS sub_sub_subcategory VARCHAR(255),
...
ADD COLUMN IF NOT EXISTS sub_sub_subcategory_id UUID REFERENCES sub3_categories(id) ON DELETE SET NULL;

-- Add indexes for better performance
CREATE INDEX IF NOT EXISTS idx_products_sub_subcategory ON products(sub_subcategory);
CREATE INDEX IF NOT EXISTS idx_products_sub_sub_subcategory ON products(sub_sub_subcategory);
CREATE INDEX IF NOT EXISTS idx_products_subcategory_id ON products(subcategory_id);
CREATE INDEX IF NOT EXISTS idx_products_sub_subcategory_id ON products(sub_subcategory_id);
CREATE INDEX IF NOT EXISTS idx_products_sub_sub_subcategory_id ON products(sub_sub_subcategory_id);

-- Composite index for hierarchical queries
CREATE INDEX IF NOT EXISTS idx_products_category_hierarchy ON products(category, subcategory, sub_subcategory, sub_sub_subcategory);

-- Update existing products to set default values based on current data
UPDATE products 
SET 
  sub_subcategory = 'General',
  sub_sub_subcategory = 'Standard'
WHERE sub_subcategory IS NULL OR sub_sub_subcategory IS NULL;

-- Add comment to document the new fields
COMMENT ON COLUMN products.sub_subcategory IS 'Level 3 category name (e.g., "iPhone 15 Pro Cases")';
COMMENT ON COLUMN products.sub_sub_subcategory IS 'Level 4 category name (e.g., "Silicon Cases")';
...
COMMENT ON COLUMN products.sub_sub_subcategory_id IS 'Foreign key to sub3_categories table';
